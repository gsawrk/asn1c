 ...
 * 
 * cc -DPDU=MyCustomType -o myDecoder.o -c j2735-dump-main.c
 */
#ifdef    HAVE_CONFIG_H
#include <config.h>
#endif
...
static FILE *argument_to_filew(char *av[], int idx);
static char *argument_to_name(char *av[], int idx);
int getHexNum(char * hexstr, int len);
...
#define    _ASX(x)    _AXS(x)
    ...
    for(num = 0; num < number_of_iterations; num++) {
      ...
      for(ac_i = (isyntax == ATS_RANDOM) ? -1 : 0; ac_i < ac; ac_i++) {
        ...
        /* Convert file contents from ASCII to hex */
        int size, hex_len;
        char * fcontent = NULL;
        ...
        hex_len = getHexNum(fcontent, size);
        printf("Hex String length input = %d, output = %d\n", size, hex_len);
        // Write hex to file
        file = argument_to_filew(av, ac_i);
        fwrite(fcontent, 1, (size/2), file);
        free(fcontent);
        fclose(file);
        // Reopen file
        file = argument_to_file(av, ac_i);
        /* END : Convert file contents from ASCII to hex */
...
static FILE *argument_to_filew(char *av[], int idx) {
    return argument_is_stdin(av, idx) ? stdin : fopen(av[idx], "w");
}
...
int getHexNum(char * hexstr, int len){
    ...
    for(i=0,j=0;i < len;i++, j++){
        ...
        if(ch < '0' || ch > 'F' || cl < '0' || cl > 'F' || (ch > '9' && ch < 'A') || (cl > '9' && cl < 'A')){
            printf("Non hex character i = %d, j = %d\n", i, j);
            return -1;
        }
        if (cl >= 'A' &&  cl <= 'F') cl -= 7;
        if (ch >= 'A' &&  ch <= 'F') ch -= 7;
